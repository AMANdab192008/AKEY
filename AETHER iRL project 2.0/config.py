...
====================
PURPOSE:
  Central place for all A.K.E.Y settings: API keys, paths, model names,
  and the AKEY system prompt. Designed for single-user use: each person runs
...
WHAT THIS FILE DOES:
  ...
  - Holds the full system prompt that defines AKEY's personality and formatting rules.
USAGE:
  Import what you need: `from config import GROQ_API_KEY, CHATS_DATA_DIR, AKEY_SYSTEM_PROMPT`
...
# Default: en-GB-RyanNeural (male British voice, fitting for AKEY).
...
# ============================================================================
# AKEY PERSONALITY CONFIGURATION
# ============================================================================
...
# Assistant name and user title: set ASSISTANT_NAME and AKEY_USER_TITLE in .env.
# The AI learns from learning data and conversation history.

ASSISTANT_NAME = (os.getenv("ASSISTANT_NAME", "").strip() or "AKEY")
AKEY_USER_TITLE = os.getenv("AKEY_USER_TITLE", "").strip()

_AKEY_SYSTEM_PROMPT_BASE = """You are {assistant_name}, a complete AI assistant — not just a chat bot. You help with information, tasks, and actions: answering questions, opening apps or websites, generating images, playing music, writing content, and searching the web. You are sharp, warm, and a little witty. Keep language simple and natural.
...
_AKEY_SYSTEM_PROMPT_BASE_FMT = _AKEY_SYSTEM_PROMPT_BASE.format(assistant_name=ASSISTANT_NAME)
if AKEY_USER_TITLE:
    AKEY_SYSTEM_PROMPT = _AKEY_SYSTEM_PROMPT_BASE_FMT + f"\n- When appropriate, you may address the user as: {AKEY_USER_TITLE}"
else:
    AKEY_SYSTEM_PROMPT = _AKEY_SYSTEM_PROMPT_BASE_FMT
